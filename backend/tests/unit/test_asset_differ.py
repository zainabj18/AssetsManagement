...
def test_removed_empty_on_match(new_assets):
    old_asset = json.loads(
        new_assets[0].json(by_alias=True, exclude={"created_at", "last_modified_at"})
    )
...
def test_change_multiple(new_assets):
    ...
    res = asset_differ(old_asset, new_asset)
    changes = [
        ["num", old_asset["num"], new_asset["num"]],
        ["name", old_asset["name"], new_asset["name"]],
        ["link", old_asset["link"], new_asset["link"]],
    ]
    for x in changes:
