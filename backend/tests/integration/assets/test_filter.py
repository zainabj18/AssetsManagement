...
def test_assets_filter_tags_AND(valid_client, new_assets):
    ...
    res = valid_client.post(
        "/api/v1/asset/filter/", json={"tags": filter_tags, "tag_operation": "AND"}
    )
...
def test_assets_filter_tags_OR(valid_client, new_assets):
    ...
    for asset in new_assets:
        if len(set(asset.tag_ids).intersection(set(filter_tags))) > 0:
            asset_ids.append(asset.asset_id)
    res = valid_client.post(
        "/api/v1/asset/filter/", json={"tags": filter_tags, "tag_operation": "OR"}
    )
    assert res.status_code == 200
    assert set(res.json["data"]) == set(asset_ids)
...
def test_assets_filter_projects_AND(valid_client, new_assets):
    ...
    res = valid_client.post(
        "/api/v1/asset/filter/",
        json={"projects": filter_project, "project_operation": "AND"},
    )
    assert res.status_code == 200
    assert set(res.json["data"]) == set(asset_ids)
...
def test_assets_filter_projects_OR(valid_client, new_assets):
    ...
    res = valid_client.post(
        "/api/v1/asset/filter/",
        json={"projects": filter_project, "project_operation": "OR"},
    )
    assert res.status_code == 200
    assert set(res.json["data"]) == set(asset_ids)
...
def test_assets_filter_classification(valid_client, new_assets):
    ...
    for asset in new_assets:
        if asset.classification.value in filter_classification:
            asset_ids.append(asset.asset_id)
    res = valid_client.post(
        "/api/v1/asset/filter/", json={"classifications": filter_classification}
    )
    assert res.status_code == 200
    assert set(res.json["data"]) == set(asset_ids)
...
def test_assets_filter_type(valid_client, new_assets):
    filter_type = [
        ...
        new_assets[1].version_id,
        new_assets[2].version_id,
    ]
...
def test_assets_AND_missing_filters(valid_client, new_assets):
    ...
    res = valid_client.post(
        ...
        json={
            "types": filter_type,
            "classifications": filter_classification,
            "operation": "AND",
        },
    )
    assert res.status_code == 200
    ...
    res = valid_client.post(
        ...
        json={
            "attributes": [
                {
                    "attributeID": -1,
                    "attributeValue": new_assets[0].name,
                    "operation": "EQUALS",
                }
            ]
        },
    ...
    res = valid_client.post(
        ...
        json={
            "attributes": [
                {
                    "attributeID": -2,
                    "attributeValue": new_assets[0].link,
                    "operation": "EQUALS",
                }
            ]
        },
    ...
    res = valid_client.post(
        ...
        json={
            "attributes": [
                {
                    "attributeID": -3,
                    "attributeValue": new_assets[0].description,
                    "operation": "EQUALS",
                }
            ]
        },
...
def test_assets_filter_attribute_has_metadata(valid_client, new_assets):
    res = valid_client.post(
        ...
        json={
            "attributes": [
                {
                    "attributeID": new_assets[0].metadata[0].attribute_id,
                    "attributeValue": None,
                    "operation": "HAS",
                }
            ]
