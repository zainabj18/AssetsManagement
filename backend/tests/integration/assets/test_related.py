...
def test_related_classification(valid_client, new_assets):
    ...
    res = valid_client.get(
        f"/api/v1/asset/related/classification/{new_assets[0].asset_id}"
    )
    assert res.status_code == 200
...
def test_related_outgoing(db_conn, valid_client, new_assets):
    with db_conn.cursor() as cur:
        cur.execute(
            ...
            {
                "from_id": new_assets[0].asset_id,
                "to_id_1": new_assets[1].asset_id,
                "to_id_2": new_assets[2].asset_id,
            },
        )
        db_conn.commit()
...
def test_related_incomming(db_conn, valid_client, new_assets):
    with db_conn.cursor() as cur:
        cur.execute(
            ...
            {
                "to_id": new_assets[0].asset_id,
                "from_id_1": new_assets[1].asset_id,
                "from_id_2": new_assets[2].asset_id,
            },
        )
        db_conn.commit()
