...
from app.db import DataAccess, Models, UserRole
...
SET type_id = %(type_id)s WHERE version_id=%(version_id)s""",
            {
                "type_id": lastest_type_version.type_id,
    ...
    """,
        ...
        cur.execute(
            """SELECT attribute_id FROM attributes_in_types WHERE type_version=%(type_version)s ;""",
            {"type_version": new_assets[0].version_id},
        )
        ...
        cur.execute(
            """SELECT attribute_name FROM attributes WHERE attribute_id=ANY(%(attribute_ids)s);""",
            {"attribute_ids": old_version_attributes_id},
        )
        ...
        cur.execute(
            """SELECT type_name FROM type_names_versions WHERE version_id=%(version_id)s;""",
            {"version_id": new_assets[0].version_id},
        )
...
def test_patch_assets_change_classification(valid_client, new_assets, db_conn):
    ...
    assert res.json["data"][0]["diff"]["added"] == []
    assert res.json["data"][0]["diff"]["removed"] == []
    assert res.json["data"][0]["diff"]["changed"][0] == [
        "classification",
        "CONFIDENTIAL",
        "PUBLIC",
    ]
...
def test_patch_assets_change_version_id(
    valid_client, new_assets, db_conn, type_verions
):
    data = json.loads(new_assets[0].json(by_alias=True))
    res = valid_client.post(f"/api/v1/asset/", json=data)
    assert res.status_code == 201
...
def test_patch_assets_change_tags(valid_client, new_assets, db_conn):
    ...
    with db_conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            """SELECT tag_id FROM assets_in_tags WHERE assets_in_tags.asset_id=%(asset_id)s;""",
            {"asset_id": asset_id},
        )
...
def test_patch_assets_change_projects(valid_client, new_assets, db_conn):
    ...
    with db_conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            """SELECT project_id FROM assets_in_projects WHERE assets_in_projects.asset_id=%(asset_id)s;""",
            {"asset_id": asset_id},
        )
...
def test_patch_assets_change_assets(valid_client, new_assets, db_conn):
    ...
    with db_conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            """SELECT to_asset_id FROM assets_in_assets WHERE assets_in_assets.from_asset_id=%(asset_id)s;""",
            {"asset_id": asset_id},
        )
...
def test_patch_assets_change_metadata(valid_client, new_assets, db_conn, type_verions):
    ...
    data["version_id"] = type_verions["added"][0].version_id
    data["metadata"] = [
        json.loads(a.json(by_alias=True)) for a in type_verions["added"][0].attributes
    ]
    res = valid_client.patch(f"/api/v1/asset/{asset_id}", json=data)
    assert res.status_code == 200
    assert res.json == {"msg": "Updated asset"}
    with db_conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            """SELECT attribute_id FROM attributes_values WHERE attributes_values.asset_id=%(asset_id)s;""",
            {"asset_id": asset_id},
        )
    ...
    added_attributes_ids = set(type_verions["added"][0].attribute_ids) - set(
        orignal_attribute_ids
    )
    removed_attributes_ids = set(orignal_attribute_ids) - set(
        type_verions["added"][0].attribute_ids
    )
    res = valid_client.get(f"/api/v1/asset/logs/{asset_id}")
    assert res.status_code == 200
    assert res.json["data"][0]["accountID"] == 1
    ...
    assert res.json["data"][0]["diff"]["changed"][0] == [
        "version_id",
        new_assets[0].version_id,
        type_verions["added"][0].version_id,
    ]
    assert res.json["data"][0]["logID"] == 2
    assert res.json["data"][0]["modelID"] == int(Models.ASSETS)
    assert res.json["data"][0]["objectID"] == asset_id
