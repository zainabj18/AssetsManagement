...
def test_comment_add_requires_asset_in_db(valid_client):
    res = valid_client.post(
        f"/api/v1/asset/comment/{1}", json={"comment": "Hello World!"}
    )
...
def test_comment_add_to_db(db_conn, valid_client, new_assets):
    comment = "Hello World!"
    res = valid_client.post(
        f"/api/v1/asset/comment/{new_assets[0].asset_id}", json={"comment": comment}
    )
...
def test_comment_multiple(db_conn, valid_client, new_assets):
    ...
    for comment in comments:
        ...
        res = valid_client.post(
            f"/api/v1/asset/comment/{new_assets[0].asset_id}", json=comment.dict()
        )
        assert res.status_code == 200
        assert res.json["msg"] == "Comment added"
...
def test_comment_delete(db_conn, valid_client, new_assets):
    ...
    assert res.status_code == 200
    res = valid_client.delete(
        f"/api/v1/asset/comment/{new_assets[0].asset_id}/remove/{1}"
    )
    assert res.status_code == 200
