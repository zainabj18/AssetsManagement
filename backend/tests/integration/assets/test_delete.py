...
from app.db import Actions, DataAccess, Models, UserRole
...
def check_db_references(db_conn, asset_id, valid_client):
    with db_conn.cursor(row_factory=dict_row) as cur:
        ...
        cur.execute(
            """SELECT * FROM assets_in_assets WHERE from_asset_id=%(asset_id)s;""",
            {"asset_id": asset_id},
        )
        assert cur.fetchone() is None
        cur.execute(
            """SELECT * FROM assets_in_assets WHERE to_asset_id=%(asset_id)s;""",
            {"asset_id": asset_id},
        )
        assert cur.fetchone() is None
        cur.execute(
            """SELECT * FROM attributes_values WHERE asset_id=%(asset_id)s;""",
            {"asset_id": asset_id},
        )
        assert cur.fetchone() is None
        cur.execute(
            """SELECT * FROM assets_in_tags WHERE asset_id=%(asset_id)s;""",
            {"asset_id": asset_id},
        )
        assert cur.fetchone() is None
        cur.execute(
            """SELECT * FROM assets_in_projects WHERE asset_id=%(asset_id)s;""",
            {"asset_id": asset_id},
        )
        assert cur.fetchone() is None
        cur.execute(
            """SELECT * FROM audit_logs WHERE object_id=%(asset_id)s AND model_id=%(model_id)s ORDER BY log_id DESC;""",
...
def test_delete_with_links(valid_client, new_assets, db_conn):
    ...
    for x in range(5, 10):
        ...
        assert res.status_code == 200
        check_db_references(
            db_conn=db_conn, asset_id=asset_ids[x - 1], valid_client=valid_client
        )
    res = valid_client.delete(f"/api/v1/asset/{first_asset_id}")
    assert res.status_code == 200
    check_db_references(
        db_conn=db_conn, asset_id=first_asset_id, valid_client=valid_client
    )
