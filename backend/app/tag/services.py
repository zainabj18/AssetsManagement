...
def tag_in_db(db: ConnectionPool, tag_id: int, abort=True):
    ...
    res = run_query(
        db,
        """SELECT id FROM tags WHERE id=%(id)s""",
        {"id": tag_id},
...
def insert_tag_to_db(db: ConnectionPool, tag: TagBase):
    ...
    return run_query(
        db,
        """
        INSERT INTO tags (name) VALUES (%(name)s) RETURNING id;""",
        {"name": tag.name},
        return_type=QueryResult.ONE,
    )
...
def list_tags(db: ConnectionPool):
    ...
    """
    return run_query(
        db, """SELECT * FROM tags ORDER BY name;""", return_type=QueryResult.ALL
    )
...
def delete_tag(db: ConnectionPool, tag_id: int):
    ...
    """
    return run_query(db, """DELETE FROM tags WHERE id=%(id)s;""", {"id": tag_id})
