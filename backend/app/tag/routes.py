...
from app.db import Actions, Models, UserRole, get_db
...
def create(user_id, access_level):
    tag = model_creator(
        model=TagBase,
        err_msg="Failed to create tag from the data provided",
        **request.json,
    )
    db = get_db()
...
def copy(user_id, access_level):
    tag_copy = model_creator(
        model=TagBulkRequest,
        err_msg="Failed to copy to tag from the data provided",
        **request.json,
    )
    db = get_db()
    if not services.tag_in_db(db, tag_copy.to_tag_id, abort=False):
        return {"msg": f"Tag {tag_copy.to_tag_id} doesn't exist"}, 400
    services.add_asset_to_tag(
        db=db, asset_ids=tag_copy.assest_ids, tag_id=tag_copy.to_tag_id
    )
    for id in tag_copy.assest_ids:
        audit_log_event(
            ...
            user_id,
            id,
            {"changed": [["tag_ids", [], [tag_copy.to_tag_id]]]},
            Actions.CHANGE,
        )
...
def remove(user_id, access_level):
    tag_remove = model_creator(
        model=TagBulkRequest,
        err_msg="Failed to remove to tag from the data provided",
        **request.json,
    )
    db = get_db()
    ...
    for id in tag_remove.assest_ids:
        audit_log_event(
            ...
            user_id,
            id,
            {"changed": [["tagIDs", [tag_remove.to_tag_id], []]]},
            Actions.CHANGE,
        )
