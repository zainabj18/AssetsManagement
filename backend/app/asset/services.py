...
from app.core.utils import QueryResult, run_query
...
def abort_asset_not_exists(db: ConnectionPool, asset_id: int):
    ...
    results = run_query(
        db,
        """SELECT asset_id FROM assets WHERE asset_id=%(id)s;""",
        {"id": asset_id},
        return_type=QueryResult.ONE,
    )
    if results is None:
...
def abort_insufficient(db: ConnectionPool, asset_id: int, access_level: DataAccess):
    ...
    results = run_query(
        db,
        """SELECT asset_id FROM assets WHERE asset_id=%(id)s AND classification<=%(access_level)s;""",
...
def insert_comment_to_db(
    db: ConnectionPool, comment: Comment, account_id: int, asset_id: int
):
    ...
    return run_query(
        ...
        {"asset_id": asset_id, "account_id": account_id, "comment": comment.comment},
    )
...
def delete_comment_db(db, comment_id):
    ...
    run_query(
        db,
        """DELETE FROM comments WHERE comment_id = %(comment_id)s;""",
...
INNER JOIN accounts ON accounts.account_id=comments.account_id
    WHERE asset_id=%(asset_id)s ORDER BY datetime;""",
        {"asset_id": asset_id},
        return_type=QueryResult.ALL_JSON,
        row_factory=class_row(CommentOut),
    )


def fetch_assets_by_common_count_count(
    db: ConnectionPool,
    asset_id: int,
    ...
    """
    query = sql.SQL(
        """WITH related_asset as (SELECT COUNT(asset_id),asset_id FROM {table} WHERE {fkey} in (SELECT {fkey} FROM {table} WHERE asset_id=%(asset_id)s) and asset_id !=%(asset_id)s
GROUP BY asset_id
...
ORDER BY count DESC;"""
    ).format(table=sql.Identifier(related_table), fkey=sql.Identifier(related_table_id))
    return run_query(
        db,
        query,
        {
        ...
        return_type=QueryResult.ALL_JSON,
        row_factory=class_row(AssetOut),
    )


def fetch_assets_by_same_attribute(
    db: ConnectionPool, asset_id: int, access_level: DataAccess, related_attribute: str
):
...
INNER JOIN type_version ON type_version.version_id=assets.version_id
INNER JOIN types ON types.type_id=type_version.type_id
WHERE assets.classification<=%(access_level)s AND assets.{attribute}=(SELECT {attribute} FROM assets WHERE asset_id=%(asset_id)s) AND asset_id!=%(asset_id)s ORDER BY asset_id;"""
    ).format(attribute=sql.Identifier(related_attribute))
    return run_query(
        db,
        query,
        {
...
INNER JOIN type_version ON type_version.version_id=assets.version_id
INNER JOIN types ON types.type_id=type_version.type_id
WHERE assets.classification<=%(access_level)s AND {to_col}=%(asset_id)s ORDER BY asset_id;"""
    ).format(from_col=sql.Identifier(from_col), to_col=sql.Identifier(to_col))
    return run_query(
        db,
        query,
        {"asset_id": asset_id, "access_level": access_level},
        return_type=QueryResult.ALL_JSON,
        row_factory=class_row(AssetOut),
    )


def fetch_assets_with_any_links(
    db: ConnectionPool, fkeys: List[int], link_table: str, fkey: str
):
    ...
    """
    query = sql.SQL(
        """SELECT asset_id FROM {table} WHERE {fkey}=ANY(%(fkeys)s);""" ""
    ).format(table=sql.Identifier(link_table), fkey=sql.Identifier(fkey))
    return run_query(db, query, {"fkeys": fkeys}, return_type=QueryResult.ALL)


def fetch_assets_with_set_links(
    db: ConnectionPool, fkeys: List[int], link_table: str, fkey: str
):
    ...
    WHERE %(fkeys)s::int[]<@ARRAY(SELECT {fkey} FROM {table} WHERE {table}.asset_id=assets.asset_id);"""
        ""
    ).format(table=sql.Identifier(link_table), fkey=sql.Identifier(fkey))
    return run_query(db, query, {"fkeys": fkeys}, return_type=QueryResult.ALL)
...
def fetch_assets_with_any_values(db: ConnectionPool, values: List[Any], attribute: str):
    ...
    """
    query = sql.SQL(
        """SELECT DISTINCT asset_id FROM assets WHERE {attribute}=ANY(%(values)s)""" ""
    ).format(attribute=sql.Identifier(attribute))
...
def fetch_assets_attribute_filter(db: ConnectionPool, searcher: AttributeSearcher):
    ...
    query = sql.Composed(
        [
            sql.SQL(
                "SELECT asset_id FROM all_atributes WHERE attribute_id=%(attribute_id)s"
            )
        ]
    )
...
def add_asset_to_db(
    ...
    version_id: int,
    description: str,
    classification: Union[str, DataAccess],
    account_id: int,
    **kwargs,
):
...
def fetch_version_dependencies(db: ConnectionPool, version_id: int):
    ...
    return run_query(
        db,
        """SELECT type_version_to as version_id,CONCAT(type_name,'-',version_number) AS type_name  FROM type_version_link
INNER JOIN type_version ON type_version.version_id=type_version_link.type_version_to
INNER JOIN types ON types.type_id=type_version.type_id WHERE type_version_from=%(version_id)s;""",
        {"version_id": version_id},
        return_type=QueryResult.ALL,
    )
...
def fetch_attributes_by_version(db: ConnectionPool, version_id: int, required=False):
    ...
    query = sql.Composed(
        [
            sql.SQL(
                """SELECT attributes_in_types.attribute_id,attributes.attribute_name FROM attributes_in_types
...
WHERE attributes_in_types.type_version=%(version_id)s"""
    ...
    if required:
        query += sql.SQL(
            " AND (attributes.validation_data->>'isOptional')::boolean is false;"
        )
...
def add_asset_metadata_to_db(
    db: ConnectionPool, asset_id: int, metadata: List[Attribute]
):
    """Inserts asset's assets into db.
...
def fetch_asset(db: ConnectionPool, asset_id: int):
    ...
    return run_query(
        db,
        """SELECT * FROM assets_out WHERE asset_id=%(asset_id)s""",
        {"asset_id": asset_id},
        row_factory=class_row(AssetOut),
        return_type=QueryResult.ONE,
    )
...
def fetch_asset_flattend(db: ConnectionPool, asset_id: int):
    ...
    return run_query(
        db,
        """SELECT * FROM flatten_assets WHERE asset_id=%(asset_id)s""",
        {"asset_id": asset_id},
        row_factory=class_row(AssetOut),
        return_type=QueryResult.ONE,
    )
...
def fetch_assets_summary(
    db: ConnectionPool, classification: DataAccess, account_id=None
):
    ...
    query = sql.Composed(
        [
            sql.SQL(
                """SELECT assets.*,type_names_versions.type_name FROM assets
...
WHERE classification<=%(classification)s"""
    ...
    return run_query(
        db,
        query,
        {"classification": classification, "account_id": account_id},
        row_factory=class_row(AssetBaseInDB),
        return_type=QueryResult.ALL_JSON,
    )
...
def fetch_assets_asssets_selected(
    db: ConnectionPool, asset_id: int, classification: DataAccess
):
...
def fetch_versions_attributes(db: ConnectionPool, version_id: int):
    ...
    return run_query(
        db,
        """SELECT attributes.* FROM attributes_in_types 
        INNER JOIN attributes ON attributes.attribute_id=attributes_in_types.attribute_id
        WHERE type_version=%(type_version)s;""",
        {"type_version": version_id},
        return_type=QueryResult.ALL_JSON,
        row_factory=class_row(AttributeInDB),
    )


def delete_projects_from_asset(
    db: ConnectionPool, asset_id: int, projects: Optional[List[int]] = None
):
    ...
    """
    query = sql.Composed(
        [sql.SQL("""DELETE FROM assets_in_projects WHERE asset_id=%(asset_id)s""")]
    )
...
def delete_tags_from_asset(
    db: ConnectionPool, asset_id: int, tags: Optional[List[int]] = None
):
    ...
    """
    query = sql.Composed(
        [sql.SQL("""DELETE FROM assets_in_tags WHERE asset_id=%(asset_id)s""")]
    )
    if tags is not None:
...
def delete_attributes_from_asset(
    db: ConnectionPool, asset_id: int, attributes: Optional[List[int]] = None
):
    ...
    """
    query = sql.Composed(
        [sql.SQL("""DELETE FROM attributes_values WHERE asset_id=%(asset_id)s""")]
    )
...
def delete_assets_from_asset(
    db: ConnectionPool, asset_id: int, asset_ids: Optional[List[int]] = None
):
    ...
    """
    query = sql.Composed(
        [sql.SQL("""DELETE FROM assets_in_assets WHERE from_asset_id=%(asset_id)s""")]
    )
...
def delete_asset(db: ConnectionPool, asset_id: int):
    ...
    """
    return run_query(
        db, """DELETE FROM assets WHERE asset_id=%(asset_id)s""", {"asset_id": asset_id}
    )
...
def fetch_asset_dependencies(db: ConnectionPool, asset_id: int):
    ...
    return run_query(
        db,
        """SELECT asset_id,name FROM assets_in_assets
INNER JOIN assets ON assets_in_assets.from_asset_id=assets.asset_id
WHERE to_asset_id=%(asset_id)s""",
        {"asset_id": asset_id},
        row_factory=class_row(AssetSummary),
        return_type=QueryResult.ALL_JSON,
    )
