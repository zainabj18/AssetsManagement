...
def create(user_id, access_level):
    ...
    audit_log_event(
        ...
        user_id,
        asset_id,
        {"added": list(Asset.schema(by_alias=True)["properties"].keys())},
        Actions.ADD,
    )
...
def my_assets(user_id, access_level):
    ...
    db = get_db()
    return {
        "data": services.fetch_assets_summary(
            db=db, classification=access_level, account_id=user_id
        )
    }
...
def list_asset_links(id, user_id, access_level):
    ...
    db = get_db()
    utils.can_view_asset(db=db, asset_id=id, access_level=access_level)
    assets = services.fetch_assets_asssets_selected(
        db=db, asset_id=id, classification=access_level
    )
    return {"data": assets}
...
def get_upgrade(id, user_id, access_level):
    ...
    new_attributes = services.fetch_versions_attributes(
        db=db, version_id=results["max_version_id"]
    )
    old_attributes = services.fetch_versions_attributes(
        db=db, version_id=results["version_id"]
    )
    new_dependencies = services.fetch_version_dependencies(
        db=db, version_id=results["max_version_id"]
    )
    new_dependencies_names = [
        depdencent["type_name"] for depdencent in new_dependencies
    ]
    added_attributes = []
    ...
    return {
        "msg": "upgrade needed",
        "data": {
            "addedAttributes": added_attributes,
            "removedAttributesNames": removed_attributes_names,
            "dependsOn": new_dependencies_names,
            "maxVersion": results["max_version_id"],
        },
    }
...
def update(id, user_id, access_level):
    ...
    db = get_db()
    utils.can_view_asset(db=db, asset_id=id, access_level=access_level)
    new_asset = model_creator(
        model=Asset,
        err_msg="Failed to create asset from the data provided",
        **request.json
    )
    old_asset = Asset(**services.fetch_asset_flattend(db=db, asset_id=id).dict())
    diff = utils.asset_differ(
        old_asset.dict(by_alias=True), new_asset.dict(by_alias=True)
    )
    ...
    attributes_removed = list(
        set([a.attribute_id for a in old_asset.metadata])
        - set([a.attribute_id for a in new_asset.metadata])
    )
    ...
    services.delete_attributes_from_asset(
        db=db, asset_id=id, attributes=attributes_removed
    )
