...
from ..services import (
    fetch_assets_by_common_count_count,
    fetch_assets_by_link,
    fetch_assets_by_same_attribute,
)
...
def fetch_related_by_tags(id: int, user_id: int, access_level: DataAccess):
    ...
    return {
        "data": fetch_assets_by_common_count_count(
            db=db,
            asset_id=id,
            access_level=access_level,
            related_table="assets_in_tags",
            related_table_id="tag_id",
        )
    }
...
def fetch_related_by_projects(id: int, user_id: int, access_level: DataAccess):
    ...
    return {
        "data": fetch_assets_by_common_count_count(
            ...
            asset_id=id,
            access_level=access_level,
            related_table="assets_in_projects",
            related_table_id="project_id",
        )
    }
...
def fetch_related_by_classification(id: int, user_id: int, access_level: DataAccess):
    ...
    can_view_asset(db=db, asset_id=id, access_level=access_level)
    return {
        "data": fetch_assets_by_same_attribute(
            db=db,
            asset_id=id,
            access_level=access_level,
            related_attribute="classification",
        )
    }
...
def fetch_related_outgoing_links(id: int, user_id: int, access_level: DataAccess):
    ...
    can_view_asset(db=db, asset_id=id, access_level=access_level)
    return {
        "data": fetch_assets_by_link(
            db=db,
            asset_id=id,
            access_level=access_level,
            to_col="from_asset_id",
            from_col="to_asset_id",
        )
    }
