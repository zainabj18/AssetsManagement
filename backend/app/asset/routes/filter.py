...
def filter():
    ...
    """
    filter = model_creator(
        model=FilterSearch,
        err_msg="Failed to run filter from the data provided",
        **request.json
    )
    db = get_db()
    ...
    if filter.tag_operation == QueryJoin.OR:
        tags_results = services.fetch_assets_with_any_links(
            db, filter.tags, link_table="assets_in_tags", fkey="tag_id"
        )
    else:
        tags_results = services.fetch_assets_with_set_links(
            db, filter.tags, link_table="assets_in_tags", fkey="tag_id"
        )
    ...
    if filter.project_operation == QueryJoin.OR:
        project_results = services.fetch_assets_with_any_links(
            db=db,
            fkeys=filter.projects,
            link_table="assets_in_projects",
            fkey="project_id",
        )
    else:
        project_results = services.fetch_assets_with_set_links(
            db, filter.projects, link_table="assets_in_projects", fkey="project_id"
        )
    filter_asset_ids.append(
        set(utils.get_key_from_results("asset_id", project_results))
    )

    # filter based on classification
    classification_results = services.fetch_assets_with_any_values(
        db=db, values=filter.classifications, attribute="classification"
    )
    filter_asset_ids.append(
        set(utils.get_key_from_results("asset_id", classification_results))
    )

    # filter based on type
    type_results = services.fetch_assets_with_any_values(
        db=db, values=filter.types, attribute="version_id"
    )
    ...
    for searcher in filter.attributes:
        ...
        filter_results = services.fetch_assets_attribute_filter(
            db=db, searcher=searcher
        )
