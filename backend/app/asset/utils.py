...
from app.schemas import Asset, Attribute, Diff
    ...
    if not asset_types.issuperset(set(get_key_from_results("version_id", dependents))):
        abort(
            400,
            {
                ...
                "data": get_key_from_results("type_name", dependents),
            },
        )


def check_asset_metatadata(
    db: ConnectionPool, version_id: int, metadata: List[Attribute]
):
    ...
    """
    required_attributes = services.fetch_attributes_by_version(
        db=db, version_id=version_id, required=True
    )
    ...
    if not set([row["attribute_id"] for row in required_attributes]).issubset(
        attribute_ids
    ):
        abort(
            400,
            {
                ...
                "data": [
                    f"Must inlcude the following attrubutes {required_attributes_names}"
                ],
            },
        )
    all_type_attributes = services.fetch_attributes_by_version(
        db=db, version_id=version_id
    )
    all_type_attributes_names = [row["attribute_name"] for row in required_attributes]
    if not (
        attribute_ids.issubset([row["attribute_id"] for row in all_type_attributes])
    ):
        abort(
            400,
            {
                ...
                "data": [
                    f"Must only inlcude the following attrubutes {all_type_attributes_names}"
                ],
            },
        )
    ...
    for key in orginal:
        if key in new:
            if key == "metadata":
                ...
                metadata_removed = list(
                    set(old_values_dict.keys()) - set(new_values_dict.keys())
                )
                metadata_added = list(
                ...
                for key in old_values_dict:
                    if key in new_values_dict:
                        if (
                            old_values_dict[key]["attributeValue"]
                            != new_values_dict[key]["attributeValue"]
                        ):
...
def add_asset_to_db(
    db: ConnectionPool, data: dict, account_id: int, asset_id: int = None
):
    ...
    """
    asset = model_creator(
        model=Asset, err_msg="Failed to create asset from the data provided", **data
    )
    db_asset = asset.dict()
    ...
    if asset_id is None:
        asset_id = services.add_asset_to_db(db=db, account_id=account_id, **db_asset)[
            "asset_id"
        ]
    else:
    ...
    services.add_asset_projects_to_db(
        db=db, asset_id=asset_id, projects=asset.project_ids
    )
